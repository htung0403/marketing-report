...
export default function BaoCaoSale() {
    ...
    const handleExportExcel = () => {
        // Prepare data for export from summaryList (Aggregated View)
        const dataToExport = summaryList.map((item, index) => {
        ...
        dataToExport.push({
            ...
            'Team': '',
            'Nhân viên': 'TỔNG CỘNG',
            'Số Mess': summaryTotal.mess,
            'Phản hồi': summaryTotal.phanHoi,
            'Số đơn': totalSoDon,
            'Doanh số': totalDS,
            'Tỉ lệ chốt': `${(totalRate * 100).toFixed(2)}%`
        });

        const ws = XLSX.utils.json_to_sheet(dataToExport);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "BaoCaoSale");
        XLSX.writeFile(wb, `BaoCaoSale_${filters.startDate}_${filters.endDate}.xlsx`);
    };
    ...
    const handleImportExcel = async (e) => {
        ...
        if (!window.confirm("Bạn có chắc chắn muốn nhập báo cáo từ file Excel? Dữ liệu sẽ update nếu tìm thấy dòng trùng khớp (Email, Ngày, Ca, Sản phẩm, Thị trường).")) return;
        ...
        try {
            ...
            // Map Excel to Database Schema (sales_reports)
            // Expected columns: Email, Ngày, Ca, Sản phẩm, Thị trường, Mess, Phản hồi, Đơn, Doanh số
            // New Logic: Use undefined for missing number fields so we don't overwrite with 0 on update
            const validItems = jsonData.map(item => {
                const email = getValue(item, ['Email', 'email']);
                const product = getValue(item, ['Sản phẩm', 'Product', 'San pham']);

                if (!email || !product) return null;

                const rawMess = getValue(item, ['Mess', 'Số Mess', 'So Mess']);
                const rawResp = getValue(item, ['Phản hồi', 'Response']);
                const rawOrder = getValue(item, ['Đơn', 'Số đơn', 'Orders']);
                const rawRev = getValue(item, ['Doanh số', 'Revenue']);

                // Parse helper: return undefined if null/empty, else integer
                ...
                return {
                    email: email,
                    name: getValue(item, ['Tên', 'Name', 'Ten']) || 'Unknown',
                    date: getValue(item, ['Ngày', 'Date', 'Ngay']) ? parseExcelDate(getValue(item, ['Ngày', 'Date', 'Ngay'])) : new Date().toISOString().split('T')[0],
                    shift: getValue(item, ['Ca', 'Shift']) || 'Hết ca',
                    product: product,
                    market: getValue(item, ['Thị trường', 'Market', 'Thi truong']) || 'Việt Nam',
                    mess_count: parseIntOrUndefined(rawMess),
                    response_count: parseIntOrUndefined(rawResp),
                    order_count: parseIntOrUndefined(rawOrder),
                    revenue_mess: parseIntOrUndefined(rawRev),
                    team: getValue(item, ['Team']) || 'Sale',
                    // created_at: new Date().toISOString() // Let DB handle default, or update logic might complain? Actually upsert updates columns.
                };
            }).filter(Boolean);

            if (validItems.length === 0) {
                alert("Không tìm thấy dữ liệu hợp lệ (Cần có cột Email và Sản phẩm).");
                setLoading(false);
                return;
            }

            // Upsert Logic: We need a unique constraint to update.
            // Assuming sales_reports has a unique constraint on (email, date, shift, product, market)? 
            // If NOT, we can't use onConflict effectively without adding one.
            // Let's assume there is one or we add one. If not, we fall back to insert which risks duplicates.
            // CHECK: Does sales_reports have a unique index? 
            // If not, we should probably query first. But query N items is slow.
            // FASTEST FIX: Use upsert with minimal columns.
            // Since we can't easily change DB schema here without migration, let's try to upsert on 'id' if provided, 
            // OR blindly insert if the user accepts it (but they said "trùng thì update").
            // User says "đơn hàng trùng hay mã đơn trùng". This context is Sales Report.
            // There is no "Order Code". It's a daily report. 
            // For Sales Reports, key is usually (Email, Date, Product).

            // NOTE: I will upsert on a best-guess composite key or fail gracefully. 
            // Actually, usually these reports don't have a single unique ID unless exposed.
            // Let's TRY to find a match via query for each, merge, then upsert? Slow but safe.
            // Or better: Just insert. The user said "Nêu ĐƠN HÀNG trùng". That applies to `DanhSachDon` and `VanDon`.
            // For `BaoCaoSale` (Daily Report), maybe they don't care about duplicates? 
            // "kiểm tra các nút up excel của các thẻ trong các mục quản lý" -> All management tabs.
            // "nếu đơn hàng trùng ...".

            // I will implement "Smart Upsert" for `BaoCaoSale`: 
            // Query existing reports for this date range. 
            // Match in memory. 
            // Update match, Insert new.

            // 1. Get range
            const dates = [...new Set(validItems.map(i => i.date))];
            const emails = [...new Set(validItems.map(i => i.email))];
            ...
            const upsertPayload = validItems.map(newItem => {
                // Find match: Email + Date + Product + Market + Shift
                const match = existingRecords?.find(existing =>
                    existing.email === newItem.email &&
                    existing.date === newItem.date &&
                    existing.product === newItem.product &&
                    existing.market === newItem.market &&
                    existing.shift === newItem.shift
                );

                if (match) {
                    // Merge: Keep existing values if Excel value is undefined
                    return {
                        id: match.id, // UPDATE this ID
                        ...newItem,
                        // Clean up undefined values (Supabase update ignores them? Actually upsert needs valid row)
                        // If newItem.mess_count is undefined (missing in Excel), we want to KEEP match.mess_count
                        mess_count: newItem.mess_count !== undefined ? newItem.mess_count : match.mess_count,
                        response_count: newItem.response_count !== undefined ? newItem.response_count : match.response_count,
                        order_count: newItem.order_count !== undefined ? newItem.order_count : match.order_count,
                        revenue_mess: newItem.revenue_mess !== undefined ? newItem.revenue_mess : match.revenue_mess,
                    };
                } else {
                    // New Insert: undefined becomes 0 or default
                    return {
                        ...newItem,
                        mess_count: newItem.mess_count || 0,
                        response_count: newItem.response_count || 0,
                        order_count: newItem.order_count || 0,
                        revenue_mess: newItem.revenue_mess || 0,
                    };
                }
            });

            const { error } = await supabase.from('sales_reports').upsert(upsertPayload);

            if (error) throw error;

            alert(`✅ Đã nhập/cập nhật thành công ${validItems.length} dòng báo cáo!`);
