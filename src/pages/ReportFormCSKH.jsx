...
function ReportFormCSKH() {
    ...
    // State for all reports, each containing all fields
    const [reports, setReports] = useState([]); // Initialized in useEffect to wait for localStorage
    ...
    // Load current user info and initialize the first report row
    useEffect(() => {
        ...
        // Initialize the first report row with user info
        setReports([{
            name: name,
            email: email,
            date: currentDate,
    ...
    const addReport = (e) => {
        ...
        const newReport = {
            name: lastReport.name,      // Auto-copy
            email: lastReport.email,    // Auto-copy
            date: lastReport.date,      // Auto-copy
            shift: lastReport.shift,    // Auto-copy
            product: lastReport.product || '', // Auto-copy product too? Maybe
            market: lastReport.market || '',   // Auto-copy market too? Maybe
            mess_cmt: '',
            response: '',
            orders: '',
            revenue: ''
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Reset but keep user info for the first row
            setReports([{
                name: defaultInfo.name,
                email: defaultInfo.email,
                date: new Date().toISOString().split('T')[0],
                shift: '', // Reset shift? Or keep? Usually reset shift for next entry logic unless same shift.
                product: '',
                market: '',
                mess_cmt: '',
                response: '',
            ...
            <div className="w-full mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 overflow-hidden">
                    <div className="mb-4 flex items-center justify-between">
                        ...
                        </h4><div className="text-sm text-gray-500 italic">
